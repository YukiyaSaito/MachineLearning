...
delta = 0.0000001
D_elem = zeros(d,d)
D_elem += delta*Matrix{Float64}(I, d, d)
D_k = zeros(d,d)
...
for k in 1:maxPasses*n
    ...
    for dind in 1:length(g_i)
        global D_elem[dind,dind] += g_i[dind]^2
        global D_k[dind, dind] = 1/(sqrt(D_elem[dind,dind]))
    end
    
    # Choose the step-size
    alpha = 1/(lambda_i*sqrt(k))

    # Take thes stochastic gradient step
    global w -= alpha*D_k*g_i
